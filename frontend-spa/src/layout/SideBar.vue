...
async function checkLoginStatus() {
  try {
    ...
    if (token && userId) {
      ...
      if (response.status === 'success' && userData && userData.user_id) {
        user.value = {
...
async function login(credentials) {
  try {
    await player.login(credentials)
  ...
  <div class="app-layout">
    <header class="mobile-header d-lg-none">
      ...
      <div class="mobile-user" @click="toggleUserMenu">
        <img v-if="isLoggedIn" :src="getAvatarUrl(user.avatar)" class="user-avatar" :alt="user.name">
        <i v-else class="fas fa-user" style="font-size: 1.5rem; color: #ff5ebc;"></i>
        <div v-if="isLoggedIn && showUserMenu" class="mobile-user-menu">
        ...
        <div class="user-profile">
          <div class="user-info" :class="{ 'collapsed': sidebarCollapsed }" @click="toggleUserMenu">
            <div class="user-avatar-container">
              <img v-if="isLoggedIn" :src="user.avatar" class="user-avatar" :alt="user.name">
              <i v-else class="fas fa-user" style="font-size: 1.5rem; color: #ff5ebc;"></i>
              <div class="online-status" v-if="isLoggedIn && user.isOnline"></div>
            </div>
            <div v-if="isLoggedIn" class="user-details" v-show="!sidebarCollapsed">
              <div class="user-name">{{ user.name }}</div>
              <div class="user-status">Premium Member</div>
            </div>
          </div>
          <!-- User Menu -->
          <div v-if="isLoggedIn && showUserMenu && !sidebarCollapsed" class="user-menu">
            <button class="user-menu-item" @click.stop="goToSettings">
              <i class="fas fa-cog"></i>
              <span>Settings</span>
            </button>
