...
const isFavorited = computed(() => {
  ...
  const isFav = favorites.value.some(fav => fav.songId === currentSong.value.id);
  console.log('Checking isFavorited:', {
    currentSongId: currentSong.value.id,
    favorites: favorites.value.map(f => ({ songId: f.songId, id: f.id })),
    result: isFav,
  });
  return isFav;
});
...
const toggleFavorite = async () => {
  if (!currentSong.value || !userId.value) return;
  try {
    if (isFavorited.value) {
      await store.removeFromFavorites(currentSong.value.id);
    } else {
      await store.addToFavorites(currentSong.value.id);
    }
...
const seekForward = () => {
  if (currentSong.value && !isLoading.value) {
    store.seek(currentSong.value.currentTime + 10);
  }
};

const seekBackward = () => {
  if (currentSong.value && !isLoading.value) {
    store.seek(Math.max(0, currentSong.value.currentTime - 10));
  }
};
