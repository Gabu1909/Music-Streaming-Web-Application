    ...
    <div class="container py-6">
      <div class="text-center mb-6">
        <h1 class="display-3 fw-bold text-white position-relative">
          ...
          <span class="title-text">Bài Hát Yêu Thích</span>
          <div class="underline"></div>
        </h1>
        <p class="lead text-light opacity-80 mt-2">Tuyển tập những giai điệu bạn yêu thích nhất</p>
      </div>
      ...
      <div v-if="loading" class="text-center py-6">
        ...
        <p class="text-light mt-4 fs-5">Đang tải danh sách yêu thích...</p>
      </div>

      <div v-else-if="error" class="alert alert-danger text-center rounded-3 shadow-sm" role="alert">
        <i class="bi bi-exclamation-circle me-2"></i>{{ error }}
        <button v-if="error.includes('đăng nhập')" class="btn btn-danger ms-3 py-1" @click="$router.push('/login')">Đăng nhập</button>
        <button v-else class="btn btn-warning ms-3 py-1" @click="fetchFavorites">Thử lại</button>
      </div>
      ...
      <div v-else-if="player?.favorites?.length === 0" class="text-center py-6">
        ...
        <p class="text-light mt-3 fs-4">Chưa có bài hát yêu thích. Hãy thêm một vài bài nhé!</p>
        <a href="/discover" class="btn btn-outline-light mt-3 px-4 py-2">Khám phá bài hát</a>
      </div>
...
const fetchFavorites = async () => {
  try {
    ...
    if (!player.userId) {
      error.value = 'Vui lòng đăng nhập để xem danh sách yêu thích.';
...
const handleToggleFavorite = (songData) => {
  player.toggleFavorite(songData.song_id, !songData.isFavorite);
};
