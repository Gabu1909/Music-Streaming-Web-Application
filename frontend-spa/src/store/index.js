...
const togglePlay = async () => {
  ...
  const login = async (credentials) => {
    try {
      ...
      const userIdFromResponse = response.user?.user_id || response.user_id;
      if (response?.token && userIdFromResponse) {
        userId.value = userIdFromResponse;
      ...
      state.error = error.message || 'Failed to log in';
      throw error;
    }
  };
  ...
  const addToFavorites = async (songId) => {
    ...
    try {
      const response = await authService.addFavorite(userId.value, songId);
  ...
  const removeFromFavorites = async (songId) => {
    ...
    try {
      ...
      if (favorite) {
        await authService.removeFavorite(userId.value, songId);
  ...
  const loadFavorites = async () => {
    if (!userId.value) throw new Error('User not logged in');
    try {
      const response = await authService.getFavorites(userId.value);
      let favoritesData = [];
